...
select customer_id,gender,location,purchase_amount_usd
from customer
where discount_applied = 'Yes' 
and purchase_amount_usd >(select avg(purchase_amount_usd) from customer);

-- 3.	Which are the top 5 products with the highest average review rating?
select item_purchased as Top_5_Products ,avg(review_rating) as Heighest_Avg_Rating
from customer
group by item_purchased
order by avg(review_rating) desc limit 5;

-- 4.	Compare the average Purchase Amounts between Standard and Express Shipping.
select shipping_type, avg(purchase_amount_usd) as average_purchased_amount
from customer 
...
select subscription_status as are_you_use_subscription ,
...
select age_group,sum(purchase_amount_usd) as "Total Revenue( usd)"
from customer
group by age_group;
